...
class ShopTest extends WrongLogTest
{
	...
	*/
	...
	{
		$this->bookkeeping =
			$this->getMock('BookKeeping',
				array('compute', 'checkLogs', 'splitLogs'),
				array(LogsStandart::getInstance())
		);
	...
	public function receiveWrongLogs($logs)
	{
		$this->bookkeeping =
			$this->getMockBuilder("Bookkeeping")
				->setConstructorArgs(array(LogsStandart::getInstance()))
				->getMock();
		$this->bookkeeping
			->expects($this->any())
			->method("compute")
	...
	*/
	public function getFilteredEntries(array $expected, $sumValue, $compareType)
	{
		...
		$this->assertEquals(
			$expected,
			$this->shop->getFilteredEntries($sumValue, $compareType)
		);
	}
	...
	/**
	* @return void
	* @expectedException 		ReportFilterException
